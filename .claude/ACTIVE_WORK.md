...
Status: idle
Task: — (last: investigated the growing Vercel "Failed preview deployment" Gmail thread — 34 messages, confirmed expected: Preview secrets are branch-scoped to master/staging only, so every other branch's Preview build has always failed by design; no recent production-deployment failures for the real project. User chose to stop it at the source rather than just filter emails — added `ignoreCommand` to vercel.json so only master/staging branches actually build; verified the shell logic for all three branch cases before committing. Takes effect on this branch as soon as pushed; needs to land on master to apply platform-wide, which per the standing pattern needs the user's own literal push command. See CURRENT_TASK.md's 2026-09-01 entry. Standing reminder unchanged: Supabase Pro + n8n + Twilio + Vapi upgrade batch remains paused — user will say when ready.)
Started: —
